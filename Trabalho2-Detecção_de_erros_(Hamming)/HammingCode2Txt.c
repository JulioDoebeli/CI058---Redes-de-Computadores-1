...
void binaryToChar(char *arr_in, char *ch_out){
	ch_out[0] = 0;
	ch_out[0] = 2 * ch_out[0] + arr_in[0]%2;
	ch_out[0] = 2 * ch_out[0] + arr_in[1]%2;
	ch_out[0] = 2 * ch_out[0] + arr_in[2]%2;
	ch_out[0] = 2 * ch_out[0] + arr_in[3]%2;
	ch_out[0] = 2 * ch_out[0] + arr_in[4]%2;
	ch_out[0] = 2 * ch_out[0] + arr_in[5]%2;
	ch_out[0] = 2 * ch_out[0] + arr_in[6]%2;
	ch_out[0] = 2 * ch_out[0] + arr_in[7]%2;
}
...
int main(int argc, char **argv){
	...
	FILE *f_in;  f_in  = fopen(argv[1], "r");  // r for read,  b for binary
	if (f_in == NULL) {
		fprintf(stdout,"Não foi aberto arquivo de entrada\n");fflush(stdout);
		return -1;
	}
	FILE *f_out; f_out = fopen(argv[2], "w");  // w for write, b for binary
	if (f_out == NULL) {
	...
	int cha = 0;
	int in_size;  in_size = ftell(f_in);
	int ed_size;  ed_size = (in_size);
	int pd_size;  pd_size = (11*in_size + 7)/15;
	int out_size; out_size= (  11*in_size + 7)/15/8;
	...
	fseek(f_in, 0L, SEEK_SET);
	cha = getc(f_in);
	for (i = 0; (i<in_size+1) && (feof(f_in) == 0); i++){
		//fprintf(stdout,"%c",cha);
		input_data_buffer[i] = (char)cha;
		//fread(&input_data_buffer[i], 1, 1, f_in);
		//fprintf(stdout,"%c",input_data_buffer[i]);
		cha = getc(f_in);
	}
		
	/*for (i = 0,j = 0; (i<in_size) && (j<ed_size); i++,j+=8){
		charToBinary(input_data_buffer[i], &encoded_data_buffer[j]);
	}*/
	for (i = 0,j = 0; (i<in_size) && (j<pd_size); i+=15,j+=11){
		int erro = hamming_15_11_block_decoder( &input_data_buffer[i], &plain_data_buffer[j]);
		if (erro != 0) fprintf(stdout, "%d %d\n", i/15, erro);
	}
	for (i = 0,j = 0; (i<pd_size) && (j<out_size); i+=8,j+=1){
		binaryToChar(&plain_data_buffer[i], &output_data_buffer[j]);
	}
	fprintf(stdout, "input_data_buffer  : %s\n",input_data_buffer); fflush(stdout);
	...
	fwrite(output_data_buffer, 1, out_size, f_out); fflush(stdout);
	//fprintf(f_out,"%s"
